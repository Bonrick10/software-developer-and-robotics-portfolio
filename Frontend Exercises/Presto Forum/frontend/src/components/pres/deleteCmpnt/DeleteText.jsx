...
export function DeleteText ({ presId, activeIndex, textIndex, setSlides }) {
  ...
  const deleteText = async () => {
    try {
      ...
      userStore.store.presentations[presIndex].slides[activeIndex].text.splice(textIndex, 1)
      storeData(userStore);
      setSlides(userStore.store.presentations[presIndex].slides);
      console.log(userStore.store.presentations[presIndex].slides[activeIndex].text);
    } catch (err) {
      alert(err.response);
    }
  }
  return (
    <>
    <Button variant="danger" onClick={deleteText}>Delete Text</Button>
    </>
  )
}
