...
public class GameMap {
    ...
    public void init() {
        ...
        initRegisterWiresAndSwitches();
        initRegisterLogicalsAndConductors();
    }
    ...
    public void initRegisterWiresAndSwitches() {
    ...
    private void initRegisterLogicalsAndConductors() {
        getEntities().stream().filter(e -> e instanceof Logical).forEach(e -> {
            ((Logical) e).findAdjConductors(this);
            ((Logical) e).initTickCounter();
        });
    }
    ...
    private void initRegisterSpawners() {
        ...
        // pass in the total count of zombie_toast_spawners
        game.setSpawnerTotalCount(zts.size());
        game.register(() -> game.getEntityFactory().spawnSpider(game), Game.AI_MOVEMENT, "spawnSpiders");
    }

    private void checkAllLogicals() {
        getEntities().stream().filter(e -> e instanceof Logical).forEach(e -> ((Logical) e).checkCondition(this));
    }
    ...
    private void triggerMovingAwayEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity && e instanceof Leaver) {
                callbacks.add(() -> ((Leaver) e).onMovedAway(this, entity));
            }
        });
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity) {
                // make sure battle with SnakeHead is triggered by player not Snake
                if (e instanceof Overlapper && !(e instanceof SnakeHead)) {
                    overlapCallbacks.add(() -> ((Overlapper) e).onOverlap(this, entity));
                } else if (entity instanceof Overlapper) {
                    overlapCallbacks.add(() -> ((Overlapper) entity).onOverlap(this, e));
                }
            }
        });
        overlapCallbacks.forEach(callback -> callback.run());
    ...
    public Distance dijkstraPathFind(Position src, Position dest, Entity entity) {
        // if inputs are invalid, don't move
        if (!nodes.containsKey(src) || !nodes.containsKey(dest))
            return new Distance(src);
        ...
        if (prev.get(ret) == null || ret.equals(src))
            return new Distance(src, 0);

        int distance = dist.get(ret);
        while (!prev.get(ret).equals(src)) {
            ret = prev.get(ret);
        }
        return new Distance(ret, distance);
    }
    ...
    public void destroyEntity(Entity entity) {
        ...
        if (entity instanceof Destroyer) {
            ((Destroyer) entity).onDestroy(this);

            List<SnakeBody> body = null;
            // if the defeated entity is a snakeHead, the whole snake is destroyed
            if (entity instanceof SnakeHead) {
                body = ((SnakeHead) entity).getBody();
            // a snakeBody part is destroyed, the rest of the body is destroyed along with that part
            } else if (entity instanceof SnakeBody) {
                body = ((SnakeBody) entity).getHead().getBody((SnakeBody) entity);
            }
            if (body != null) {
