...
public class BattleStatistics {
    ...
    // since battle response is always triggered by player, self would always be player, target would always be enemy
    public static List<BattleRound> battle(BattleStatistics self, BattleStatistics target) {
        // the only instance where target.invincible is true is when enemy is a snakeBody or snakeHead
        // when snake is in invincible state, it still triggers a normal battle reponse
        List<BattleRound> rounds = new ArrayList<>();
        if (self.invincible) {
            double damageOnSelf = (self.invincible) ? 0 : self.getHealth();
            double damageOnTarget = self.getHealth();
