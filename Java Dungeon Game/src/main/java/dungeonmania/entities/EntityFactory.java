...
import dungeonmania.entities.logicals.condtions.LogicFactory;
...
public class EntityFactory {
    ...
    private SnakeHead buildSnakeHead(Position pos) {
        double snakeAttack = config.optDouble("snake_attack", SnakeHead.DEFAULT_HEALTH);
        double snakeAttackArrowBuff = config.optDouble("snake_attack_arrow_buff", SnakeHead.DEFAULT_ATTACK);
        double snakeHealth = config.optDouble("snake_health", SnakeHead.DEFAULT_BUFF);
        double snakeHealthTreasureBuff = config.optDouble("snake_health_treasure_buff", SnakeHead.DEFAULT_BUFF);
        double snakeHealthKeyBuff = config.optDouble("snake_health_key_buff", SnakeHead.DEFAULT_BUFF);
        return new SnakeHead(pos, snakeHealth, snakeAttack, snakeAttackArrowBuff, snakeHealthTreasureBuff,
            snakeHealthKeyBuff, new BaseState(), new ArrayList<SnakeBody>());
    }

    private Bomb buildBomb(Position pos, JSONObject jsonEntity) {
        int bombRadius = config.optInt("bomb_radius", Bomb.DEFAULT_RADIUS);
        if (jsonEntity.has("logic")) {
            LogicRule condition = LogicFactory.createLogic(jsonEntity.getString("logic"));
            return new LogicBomb(pos, bombRadius, condition);
        }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "bomb":
            return buildBomb(pos, jsonEntity);
