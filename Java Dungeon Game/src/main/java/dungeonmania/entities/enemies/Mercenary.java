...
import dungeonmania.entities.enemies.enemyMoveStrategy.MercAllyStrat;
...
public class Mercenary extends Enemy implements Interactable {
    ...
    public void move(Game game) {
        ...
        if (allied) {
            setMoveStrat(new MercAllyStrat());
            nextPos = executeStrat(game);
        } else if (map.getPlayer().getEffectivePotion() instanceof InvisibilityPotion) {
            setMoveStrat(new RandomStrat());
            nextPos = executeStrat(game);
        } else if (map.getPlayer().getEffectivePotion() instanceof InvincibilityPotion) {
            setMoveStrat(new InvincibleStrat());
            nextPos = executeStrat(game);
        } else {
            // Follow hostile
            nextPos = map.dijkstraPathFind(getPosition(), player.getPosition(), this).getPosition();
        }
