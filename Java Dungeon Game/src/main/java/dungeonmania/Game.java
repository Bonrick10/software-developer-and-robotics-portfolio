...
public class Game {
    ...
    private int spawnerTotalCount = 0;
    ...
    public void battle(Player player, Enemy enemy) {
        ...
        if (enemy.getBattleStatistics().getHealth() <= 0) {
            // if the defeated snakeHead is in invincible state, nothing happens
            if (enemy instanceof SnakeHead) {
                if ((((SnakeHead) enemy).getState()) instanceof InvincibleState) {
                     return;
                }
            // if the defeated snakeBody is in invincible state, a new snake is created
            } else if (enemy instanceof SnakeBody) {
                if ((((SnakeBody) enemy).getState()) instanceof InvincibleState) {
                    ((SnakeBody) enemy).getHead().growNewHead(map, (SnakeBody) enemy);
                    return;
                }
            }
    ...
    public int tick() {
        ...
        sub = nextTickSub;
        tickCount++;
        map.getEntities().stream().filter(e -> e instanceof Logical).forEach(e -> ((Logical) e).updateTickCounter());
        return tickCount;
    }
    ...
    public void setSpawnerTotalCount(int count) {
        this.spawnerTotalCount = count;
    }
    ...
    public int getSpawnerDestroyCount() {
        return player.getSpawnerDestroyCount();
    }

    public void addSpawnerDestroyCount() {
        player.addSpawnerDestroyCount();
    }

    public int getSpawnerTotalCount() {
        return spawnerTotalCount;
    }
